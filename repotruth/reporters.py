...
def sarif_report(result: ScanResult) -> str:
    ...
    payload = {
        ...
        "version": "2.1.0",
        "runs": [{
            "tool": {"driver": {"name": "RepoTruth", "version": "0.4.0", "informationUri": "https://github.com/flokiss42-source/repotruth", "rules": list(rules.values())}},
            "results": [{
                "ruleId": item.rule_id,
...
def html_report(result: ScanResult) -> str:
    cards = []
    for item in result.findings:
        evidence = f'<p><b>Evidence:</b> <code>{html.escape(item.evidence)}</code></p>' if item.evidence else ''
        cards.append(f'''<article class="finding {html.escape(item.severity)}"><div class="meta"><b>{html.escape(item.severity.upper())}</b> · {html.escape(item.rule_id)} · {html.escape(item.path)}:{item.line}</div><h3>{html.escape(item.title)}</h3><p>{html.escape(item.message)}</p>{evidence}<p class="fix"><b>Fix:</b> {html.escape(item.remediation)}</p></article>''')
...
</style></head><body><main><header><div><h1>RepoTruth</h1><div class="sub">Security and evidence report for <code>{html.escape(result.root.name)}</code> · grade {result.grade}</div></div><div class="score">{result.score}</div></header>{content}</main></body></html>'''
